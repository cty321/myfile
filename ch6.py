...
class Fridge:
    """This class implements a fridge where ingredients can be added and
    removed individually, or in groups.
    The fridge will retain a count of every ingredient added or removed, and will
    ...
    has_varrious(foods) - checks if enough of every food in th dictionary is in
    the fridge
    ...
    def add_one(self,food_name):
        """
        add_one(food_name) - adds a single food_name to fridge returns Ture
    ...
    def add_many(self,food_dict):
        """
        add_many(food_dict) - adds a whole dictionaty filled with food as keys
        and quantities as values.
        returns a dictionaty with the removed food.
        raises a TypeError if food_dict is not a dictionaty
    ...
    def has(self,food_name,quantity=1):
        """
        has(food_name,[quantity]) - checks is the string food_name is in the
        fredge. Quantity defaults to 1
        Returns Ture if there is enough,False otherwise."""

        return self.has_various({food_name:quantity})

    def has_various(self,foods):
        """
        has_various(foods) determines if the dictionary food_name has enough of
        every element to satisfy a request.
        returns Ture if there's enough, False if there's not or if an element does
        not exist."""
    ...
    def __get_multi(self,food_name,quantity):
        ...
        Returns the number of items removed returns False if there isn't enough
    ...
    def get_one(self,food_name):
        """
        get_one(food_name) - takes out a single food_name from the fridge returns
        a dictionary with the food:1 as a result,or False if there wasn't
    ...
    def get_many(self,food_dict):
        ...
        returns False if there are not enough ingredients or if a dictionary isn't
        provided."""
...
class Omelet:
    """This class creates an omelet object. An omelet can be in one of two states:
    ingredients,or cooked.
    ...
    mix() - gets called ofter all the ingredients are gathered from the fridge
    ...
    def __ingredients__(self):
        """Internal method to be called on by a fridge or other objects that need
        to act on ingredients. """
